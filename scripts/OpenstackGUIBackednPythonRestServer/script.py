...
def list_instances():
    ...
    for key in servers:
        if len(key.addresses['private']) > 1:
            out.append({'instance_id': key.id, 'instance_name':key.name, 'status':key.status, 'flavor':key.flavor['id'],'image_id':key.image['id'],'public_ip':key.addresses['private'][1]['addr'],'private_ip':key.addresses['private'][0]['addr']})
        else :
            out.append({'instance_id': key.id, 'instance_name':key.name, 'status':key.status, 'flavor':key.flavor['id'],'image_id':key.image['id'],'public_ip':"",'private_ip':key.addresses['private'][0]['addr']})
    response.content_type = 'application/json'
...
def list_types():
    ...
    out = [{'flavor_id': key.id, 'flavor_name':key.name,'ram':key.ram,'vcpus':key.vcpus,'disk':key.disk} for key in flavors]
